...
void StratumClient::connect()
{
    ...
    m_resolver.async_resolve(q,
            boost::bind(&StratumClient::resolve_handler,
...
void StratumClient::reset_work_timeout()
{
    ...
    m_worktimer.async_wait(boost::bind(&StratumClient::work_timeout_handler, this, boost::asio::placeholders::error));
}

void StratumClient::start_connect(tcp::resolver::iterator endpoint_iter)
{
    if (endpoint_iter != tcp::resolver::iterator()) {
        ...
        m_socket->async_connect(endpoint_iter->endpoint(),
                boost::bind(&StratumClient::connect_handler, this, _1, endpoint_iter));
    } else {
...
void StratumClient::connect_handler(const boost::system::error_code& ec, tcp::resolver::iterator i)
{
...
void StratumClient::submitSolution(Solution solution)
{
    ...
    m_responsetimer.async_wait(boost::bind(&StratumClient::response_timeout_handler, this, boost::asio::placeholders::error));
...
void StratumClient::recvSocketData()
{
    if (m_conn.SecLevel() != SecureLevel::NONE) {
        async_read_until(*m_securesocket, m_recvBuffer, "\n",
                boost::bind(&StratumClient::onRecvSocketDataCompleted, this,
...
void StratumClient::sendSocketData(Json::Value const & jReq)
{
    ...
    if (m_conn.SecLevel() != SecureLevel::NONE) {
        async_write(*m_securesocket, m_sendBuffer,
                boost::bind(&StratumClient::onSendSocketDataCompleted, this, boost::asio::placeholders::error));
    } else {
