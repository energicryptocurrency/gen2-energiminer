...
private:
    ...
    T* indirect_ptr(difference_type pos) { return reinterpret_cast<T*>(_union.cap.indirect) + pos; }
    const T* indirect_ptr(difference_type pos) const { return reinterpret_cast<const T*>(_union.cap.indirect) + pos; }
    ...
    void change_capacity(size_type new_capacity) {
        if (new_capacity <= N) {
            ...
            if (!is_direct()) {
                _union.cap.indirect = static_cast<char*>(realloc(_union.cap.indirect, ((size_t)sizeof(T)) * new_capacity));
                _union.cap.capacity = new_capacity;
            } else {
                char* new_indirect = static_cast<char*>(malloc(((size_t)sizeof(T)) * new_capacity));
                T* src = direct_ptr(0);
                T* dst = reinterpret_cast<T*>(new_indirect);
                memcpy(dst, src, size() * sizeof(T));
                _union.cap.indirect = new_indirect;
                _union.cap.capacity = new_capacity;
                _size += N + 1;
            }
        }
...
public:
    ...
    ~prevector() {
        clear();
        if (!is_direct()) {
            free(_union.cap.indirect);
            _union.cap.indirect = NULL;
        }
    }
    ...
    size_t allocated_memory() const {
        if (is_direct()) {
            ...
            return ((size_t)(sizeof(T))) * _union.cap.capacity;
        }
    }
};
#pragma pack(pop)
